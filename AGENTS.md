...
python -m cli process --lyrics lyrics.txt --mp3 song.mp3 --title "Title" --artist "Artist" [--video song.mp4] [--cover cover.jpg] [--output ./dir]
...
Each processing run writes into its own per-song folder `<output_dir>/<artist> - <title>/`, and **all** output files use the sanitized `<artist> - <title>` base name (`cli/pipeline.sanitize_output_name`, built on `sanitize_filename`): `.txt`, `.mp3`, video (same extension), `.zip`, `.html` (preview), `_editor.html`, `_vocals.mp3`, `_accompaniment.mp3`, `_cover.<ext>` (optional JPEG cover, e.g. `_cover.jpg`). The generated `.txt`'s `#MP3`/`#VIDEO`/`#COVER` tags and the editor's audio hint reference these names. `--no-intermediates` (CLI) keeps the ZIP to the package output only; by default the ZIP also bundles the job's `tmp/` artifacts under `intermediates/` (`cli/package.collect_intermediates`).
...
- **Header**: leading `#` lines are preserved verbatim (incl. `#COVER`/`#BACKGROUND`); CRLF→LF on save. The body is serialized in the same shape as `build_ultrastar_txt`. On save the editor stamps/refreshes `#CREATOR:https://github.com/Beherith/ultrasongs  - <ISO-8601 UTC timestamp>` (inserted after `#ARTIST`, or appended when absent), replacing any existing `#CREATOR` line. The pipeline does the same stamping in `generate_ultrastar`/`build_ultrastar_txt` (`UltrastarMeta.creator`, `cli.ultrastar.creator_value`).
...
| `cli/medley.py` | `detect_medley()`: locate the medley/chorus in plain lyrics — the block of ≥2 consecutive lines that occurs most often (ties: longer block, then earliest); returns the line span of its first occurrence; `normalize_lyric_line()` |
| `cli/syllabify.py` | Syllable splitting via pyphen (language alias map, cached hyphenators) |
| `cli/ultrastar.py` | `ms_to_beats()`, `build_ultrastar_txt()`, `parse_ultrastar_txt()`, `extract_lyrics_from_ultrastar()`; writes/parses the optional `#MEDLEYSTARTBEAT`/`#MEDLEYENDBEAT`/`#PREVIEWSTART` tags |
...
4. **Generate** (`cli/generate.py`): aligned syllables + BPM → Ultrastar `.txt`, beat grid anchored to the detected first beat (`#GAP` = `first_beat_ms`, fallback `first note - gap_lead_in_ms`), shifted back by a whole number of grid beats to just before the first note when it would land after it (keeping the detected BPM phase; clamped at 0 since a negative `#GAP` is not allowed; negative note beats are not allowed), exported BPM scaled by `beat_resolution_multiplier`. The medley (chorus) is located in the supplied lyrics (`cli/medley.detect_medley`: the block of ≥2 lines that repeats most often, ties go to the longer/earlier block) and written as `#MEDLEYSTARTBEAT`/`#MEDLEYENDBEAT` (beat span of the first occurrence) plus `#PREVIEWSTART` (seconds from audio start = `#GAP` + start beat × beat length, comma decimal). All three tags are omitted when no block repeats
...
| `cli/tests/test_medley.py` | `normalize_lyric_line()`, `detect_medley()` (repeat/first-occurrence/tie-break rules, real test-song lyrics) |
