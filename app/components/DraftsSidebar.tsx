...
export default function DraftsSidebar({ open, onToggle, onLoad, onImport, refreshKey, currentDraftId }: Props) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-30 flex h-screen flex-col overflow-y-auto border-r border-zinc-200 bg-zinc-50 transition-all duration-200 dark:border-zinc-700 dark:bg-zinc-900 ${
        open ? "w-64" : "w-10"
      }`}
    >
      {/* Toggle button */}
      <button
        onClick={onToggle}
        title={open ? "Ocultar borradores" : "Mostrar borradores"}
        className="flex h-10 w-full items-center justify-center border-b border-zinc-200 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 dark:border-zinc-700 dark:hover:bg-zinc-800 dark:hover:text-zinc-300"
      >
        {open ? (
      ...
      {open && (
        <>
            ...
            {showImport && (
              <div className="mt-1.5 flex flex-col gap-1.5 rounded-lg border border-zinc-200 bg-white p-2 dark:border-zinc-700 dark:bg-zinc-800">
                <label className="text-[10px] text-zinc-500">
                  Archivo .txt *
                  <input ref={txtRef} type="file" accept=".txt" className="mt-0.5 block w-full text-[10px] text-zinc-600 file:mr-1 file:rounded file:border-0 file:bg-zinc-100 file:px-1.5 file:py-0.5 file:text-[10px] dark:file:bg-zinc-700 dark:file:text-zinc-300"
                    onChange={(e) => setImportTxt(e.target.files?.[0] ?? null)} />
                </label>
                <label className="text-[10px] text-zinc-500">
                  Archivo .mp3 *
                  <input ref={mp3Ref} type="file" accept="audio/*" className="mt-0.5 block w-full text-[10px] text-zinc-600 file:mr-1 file:rounded file:border-0 file:bg-zinc-100 file:px-1.5 file:py-0.5 file:text-[10px] dark:file:bg-zinc-700 dark:file:text-zinc-300"
                    onChange={(e) => setImportMp3(e.target.files?.[0] ?? null)} />
                </label>
                <label className="text-[10px] text-zinc-500">
                  Video (opcional)
                  <input ref={vidRef} type="file" accept="video/*" className="mt-0.5 block w-full text-[10px] text-zinc-600 file:mr-1 file:rounded file:border-0 file:bg-zinc-100 file:px-1.5 file:py-0.5 file:text-[10px] dark:file:bg-zinc-700 dark:file:text-zinc-300"
          ...
          <div className="flex flex-col gap-1 overflow-y-auto px-2 pb-4">
            {drafts.length === 0 && (
              <p className="px-1 py-4 text-center text-xs text-zinc-400">Sin borradores</p>
            )}
            {drafts.map((d) => (
              <div
                key={d.id}
              ...
              >
                ...
                <div className="mt-1 flex items-center gap-1.5">
                  <span className="text-[10px] text-zinc-400">{fmt(d.savedAt)}</span>
                  {d.hasVideo && (
